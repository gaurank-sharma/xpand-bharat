        ...
        <div className="bg-[#0f0f0f] border border-[#1e1e1e] rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {insights.map((ins) => (
                  <tr key={ins._id} className="border-b border-[#161616] hover:bg-white/2 transition-colors">
                    ...
                    <td className="px-5 py-3 text-gray-400 text-xs whitespace-nowrap">{ins.tag}</td>
                    <td className="px-5 py-3 text-gray-400 text-xs whitespace-nowrap">{ins.displayDate}</td>
                    <td className="px-5 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full border capitalize ${statusBadge[ins.status]}`}>{ins.status}</span>
                    </td>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => toggleStatus(ins)} title={ins.status === 'published' ? 'Unpublish' : 'Publish'} className={`p-1.5 transition-colors ${ins.status === 'published' ? 'text-green-400 hover:text-gray-400' : 'text-gray-500 hover:text-green-400'}`}>
