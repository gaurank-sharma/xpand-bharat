  ...
  area ? (
    <div>
      ...
      <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={3}
        className="w-full bg-[#ffffff] border border-[#d4d7dd] text-[#0b1430] placeholder-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#f07920] resize-none" />
    </div>
  ) : (
    <div>
      <label className="block text-gray-600 text-xs mb-1">{label}</label>
      <input type="text" value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className="w-full bg-[#ffffff] border border-[#d4d7dd] text-[#0b1430] placeholder-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#f07920]" />
    </div>
  );
...
export default function AdminPages() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="relative">
          <select value={page} onChange={e => setPage(e.target.value)}
            className="bg-[#ffffff] border border-[#e6e8ec] text-[#0b1430] rounded-lg pl-4 pr-10 py-2.5 text-sm focus:outline-none focus:border-[#f07920] appearance-none cursor-pointer">
            {PAGES.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
          </select>
          <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none" />
        </div>
        {/* Tabs */}
        <div className="flex gap-1 ml-auto bg-[#ffffff] border border-[#e6e8ec] rounded-lg p-1">
          {['hero', 'content'].map(t => (
            <button key={t} onClick={() => setTab(t)} className={`px-4 py-1.5 rounded-md text-sm font-medium capitalize transition-all ${tab === t ? 'bg-[#f07920] text-white' : 'text-gray-600 hover:text-[#0b1430]'}`}>{t}</button>
          ))}
        </div>
      </div>
      ...
      {/* ── HERO TAB ── */}
      {tab === 'hero' && hero && (
        <div className="bg-[#ffffff] border border-[#e6e8ec] rounded-xl p-6 space-y-4">
          <h3 className="text-[#0b1430] font-medium text-sm mb-2">Hero Section — <span className="text-[#f07920]">{PAGES.find(p => p.value === page)?.label}</span></h3>
          ...
          {/* Background image */}
          <div>
            <label className="block text-gray-600 text-xs mb-2">Background Image</label>
            <label className={`flex items-center justify-center border-2 border-dashed rounded-xl cursor-pointer h-40 transition-colors overflow-hidden ${heroImgPrev ? 'border-[#f07920]/30' : 'border-[#d4d7dd] hover:border-[#f07920]/30'}`}>
              {heroImgPrev
            ...
            <input type="text" value={!heroImg ? (hero.backgroundImage || '') : ''} onChange={e => { setHero(h => ({ ...h, backgroundImage: e.target.value })); setHeroImgPrev(e.target.value); setHeroImg(null); }}
              ...
              className="w-full mt-2 bg-[#ffffff] border border-[#d4d7dd] text-[#0b1430] placeholder-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#f07920]" />
          </div>
      ...
      {tab === 'content' && (
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="relative">
              <select value={section} onChange={e => setSection(e.target.value)}
                className="bg-[#ffffff] border border-[#e6e8ec] text-[#0b1430] rounded-lg pl-4 pr-10 py-2 text-sm focus:outline-none focus:border-[#f07920] appearance-none cursor-pointer">
          ...
          {visibleItems.length === 0 ? (
            <div className="text-center py-16 bg-[#ffffff] border border-[#e6e8ec] rounded-xl text-gray-500 text-sm">No items in this section.</div>
          ) : (
            <div className="bg-[#ffffff] border border-[#e6e8ec] rounded-xl overflow-hidden">
              <div className="px-4 py-2 border-b border-[#e6e8ec] flex items-center gap-2">
                <span className="text-gray-500 text-xs">{activeCount} of {visibleItems.length} visible on site</span>
              </div>
              <table className="w-full text-sm">
                <thead><tr className="border-b border-[#e6e8ec]">
                ...
                <tbody>
                  {visibleItems.map(item => {
                    ...
                    return (
                      <tr key={item._id} className={`border-b border-[#eef0f3] transition-colors ${isOn ? 'hover:bg-[#f8f9fb]' : 'opacity-40 hover:opacity-60'}`}>
                        ...
                        <td className="px-4 py-3">
                          {item.imageUrl
                            ...
                            : <div className="w-12 h-9 bg-[#ffffff] rounded flex items-center justify-center text-gray-600 text-xs">{item.badge || '—'}</div>}
                        </td>
                        <td className="px-4 py-3">
                          {item.badge && <span className="text-[#f07920] text-xs font-mono block">{item.badge}</span>}
                          {item.tag  && <span className="text-gray-600 text-xs">{item.tag}</span>}
                        </td>
                        <td className="px-4 py-3 text-[#0b1430] text-sm max-w-[200px]">
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full transition-all ${isOn ? 'bg-emerald-500/10 text-emerald-400 hover:bg-red-500/10 hover:text-red-400' : 'bg-[#ffffff] text-gray-500 hover:bg-emerald-500/10 hover:text-emerald-400'}`}
                          >
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#ffffff] border border-[#e6e8ec] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#e6e8ec]">
              <h3 className="text-[#0b1430] font-semibold">{editing ? 'Edit Item' : 'Add Item'} — <span className="text-[#f07920] text-sm">{SECTION_LABELS[section] || section}</span></h3>
            ...
            <form onSubmit={saveItem} className="p-6 space-y-4">
              ...
              {shows(page,section,'image') && <div>
                <label className="block text-gray-600 text-xs mb-2">Image <span className="text-gray-600">(uploads saved to Cloudinary permanently)</span></label>
                <label className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl cursor-pointer h-28 transition-colors overflow-hidden ${imgPrev ? 'border-[#f07920]/30' : 'border-[#d4d7dd] hover:border-[#f07920]/30'}`}>
                  {imgPrev
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setModal(false)} className="flex-1 py-2.5 border border-[#d4d7dd] text-gray-600 rounded-lg text-sm hover:border-[#c8ccd3]">Cancel</button>
